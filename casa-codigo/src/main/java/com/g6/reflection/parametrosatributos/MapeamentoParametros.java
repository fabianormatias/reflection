package com.g6.reflection.parametrosatributos;
...
public class MapeamentoParametros<E> {
	...
	private Method getSetter(Field f) {
		...
		throw new RuntimeException(nomeMetodo + "() não encontrado");
	}

	private void inserir(E instancia, String nomeParametro, List<String> valores) {
		Field f = parametros.get(nomeParametro);
		if (f == null) {
			throw new RuntimeException(nomeParametro + ": parâmetro não previsto.");
		}
		...
		try {
			...
			throw new RuntimeException("Problemas ao invocar " + m.getName(), e);
		}
	}
	...
	private Object recuperarValor(String nomeParametro, List<String> valores, Method m) {
		if (m.getParameterTypes()[0] == boolean.class || m.getParameterTypes()[0] == Boolean.class) {
			if (valores.size() > 0) {
				throw new RuntimeException(nomeParametro + " não pode possuir valor.");
			} else {
				return true;
			}
			...
			if (valores.size() != 1) {
				throw new RuntimeException(nomeParametro + " só pode possuir um valor.");
			} else {
	...
	public E mapear(String[] args) {
		...
		try {
			...
			throw new RuntimeException(clazz.getName() + " não pode ser instanciada ", e);
		}
